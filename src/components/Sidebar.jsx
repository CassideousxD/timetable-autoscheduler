...
const Sidebar = () => {
  ...
  const navigation = [
    ...
    },
    {
      ...
      path: '/faculty-course-assignments',
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
            ...
            <nav className="mt-5 flex-1 px-2 space-y-1">
              {navigation.map((item) => (
                ...
                >
                  <div className="mr-3 flex-shrink-0 h-6 w-6 text-gray-400 group-hover:text-gray-300">
                    {item.icon}
                  </div>
                  {item.name}
                </NavLink>
              ))}
              {user?.role === 'timetable_coordinator' && (
                <NavLink
                  to="/timetable-coordinator-dashboard"
                  className={({ isActive }) =>
                    `group flex items-center px-2 py-2 text-sm font-medium rounded-md ${
                      isActive
                        ? 'bg-gray-900 text-white'
                ...
                >
                  ...
                  Timetable Coordinator Dashboard
                </NavLink>
              )}
            </nav>
          </div>
          <div className="flex-shrink-0 flex border-t border-gray-700 p-4">
            <div className="flex-shrink-0 w-full group block">
              <div className="flex items-center">
                <div>
                  <div className="inline-flex items-center justify-center h-10 w-10 rounded-full bg-gray-700">
